...
-- Q Write a SQL query to find the actors who were cast in the movie 'Annie Hall'. Return actor first name, last name and role.
...
-- Q. From the following tables, write a SQL query to find the director who directed a movie that casted a role for 'Eyes Wide Shut'.
...
JOIN movie
   ON movie.mov_id = movie_cast.mov_id AND movie.mov_title = 'Eyes Wide Shut'

-- Q. Write a SQL query to find who directed a movie that casted a role as ‘Sean Maguire’. Return director first name, last name and movie title
-- ANSWER 1 
SELECT dir_fname, dir_lname, mov_title 
FROM director 
INNER JOIN movie_direction 
...
INNER JOIN movie_cast 
   ON movie_cast.mov_id = movie.mov_id AND movie_cast.role = 'sean maguire';
-- ANSWER 2 
SELECT dir_fname, dir_lname, movie.mov_title 
...
-- Q. Write a SQL query to find the actors who have not acted in any movie between 1990 and 2000 (Begin and end values are included.).
...
-- Q. Write a SQL query to find the directors with the number of genres of movies. Group the result set on director first name, last name 
...
ORDER BY dir_fname, dir_lname;
 ...
 -- Q. Write a SQL query to find the movies with year and genres. Return movie title, movie year and generic title.
SELECT mov_title, mov_year, gen_title 
FROM movie
NATURAL JOIN  movie_genres
NATURAL JOIN genres;

-- Q. Write a SQL query to find the movies released before 1st January 1989. Sort the result-set in descending order by date of release.
...
-- Q. write a SQL query to find all the movies with year, genres and the name of the directors
SELECT  mov_title, mov_time, gen_title, dir_fname, dir_lname from movie 
NATURAL JOIN movie_genres
NATURAL JOIN genres 
NATURAL JOIN movie_direction
NATURAL JOIN director

-- Q. Write a SQL query to compute the average time and count the number of movies for each genre. Return genre title, average time
--     and the number of movies for each genre.
SELECT AVG(mov_time), count(gen_title), gen_title
FROM movie 
NATURAL JOIN movie_genres
NATURAL JOIN genres
GROUP BY gen_title

-- Q. Write a SQL query to find movies with the lowest duration. Return movie title, movie year, director first name+last name, 
--      , actor first name+last name and role.
SELECT mov_title, mov_year, concat(dir_fname,' ', dir_lname) AS Director_name , 
concat(act_fname,' ', act_lname) AS actor_name, role 
FROM movie 
NATURAL JOIN  movie_direction
